/** @module AdminInactiveTable — Sortable, paginated table for inactive admins with enable/disable actions. */
...
/**
 * Renders a sticky-header table of inactive admins with sort, pagination and row actions.
 * @param {object} props - Table data, pagination state and action callbacks.
 * @returns {JSX.Element}
 */
const AdminInactiveTable = ({ loading, adminData, totalItems, page, rowsPerPage, setPage, setRowsPerPage, setSortBy, setSortOrder, deleteAdmin, handleEdit }) => {
  ...
  if (!adminData?.length) return <EmptyState subtitle='No inactive admins found.' />
