...
const useCompanyData = () => {
    ...
    const addCompany = async (newCompany) => {
        try {
            const response = await axios.post("https://hrm.backhrm.online/api/add-company", {
                ...newCompany
            }, {
                headers: {
    ...
    const editCompany = async (updatedData, companyId) => {
        try {
            const response = await axios.put(`https://hrm.backhrm.online/api/update-company/${companyId}`, updatedData, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                    Authorization: `Bearer ${authToken?.token}`
                }
    ...
    const deleteCompany = async (id) => {
        try {
            const response = await axios.delete(`https://hrm.backhrm.online/api/delete-company/${id}`, {
                headers: {
                    Authorization: `Bearer ${authToken?.token}`,
                },
            });
        ...
        const fetchCompanyLogos = async () => {
            try {
                ...
                await Promise.all(companyData.map(async (company) => {
                    const response = await axios.get(`https://hrm.backhrm.online/api/company-logo/${company.id}`, {
